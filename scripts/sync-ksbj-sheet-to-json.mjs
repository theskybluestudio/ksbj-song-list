...
function getSheetId() {
  ...
  if (!sheetId) {
    throw new Error("Missing KSBJ_SHEET_ID or KSBJ_SONGS_CSV_URL in .env.local");
  }
  return sheetId;
}
...
async function main() {
  ...
  const appCachePath = path.join(appRoot, "data", "ksbj-songs.json");
  const sourceCachePath = path.join(projectRoot, "sources", "ksbj", "data", "app-songs.json");
